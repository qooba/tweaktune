...
class JudgeRatings(BaseModel):
    ...
    relevance: float = Field(
        description="Relevance values (0-2): relevance of the text to the prompt"
    )
    clarity: float = Field(
        description="Clarity values (0-2): clarity of the language used in the text"
    )
...
class UnslothWrapper(LLMWrapper):
    ...
    def process(
        ...
        output_ids = self.model.generate(
            input_ids=inputs, max_new_tokens=max_tokens, temperature=temperature, use_cache=True
        )
